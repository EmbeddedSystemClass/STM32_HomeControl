...
const char temp_aussen[] 			__FLASH__ = "Temp-Aussen";
const char temp_luft_ein[]			__FLASH__ = "Temp-Luft-Ein";
const char temp_luft_aus[]			__FLASH__ = "Temp-Luft-Aus";
const char temp_hzg_vl[]			__FLASH__ = "Temp-Hzg-VL";
const char temp_hzg_rl[]			__FLASH__ = "Temp-Hzg-RL";
const char temp_ww[]				__FLASH__ = "Temp-WW";
const char temp_ww_soll[]			__FLASH__ = "Temp-WW-Soll";
const char status_verdichter[]		__FLASH__ = "Status-Verdichter";
const char status_pumpe_hzg[]		__FLASH__ = "Status-Pumpe-Hzg";
const char status_pumpe_ww[]		__FLASH__ = "Status-Pumpe-WW";
const char verdichter_betriebstd[]	__FLASH__ = "Verdichter-BetriebStd";
const char verdichter_nrstarts[]	__FLASH__ = "Verdichter-NrStarts";
const char strom_wp[]				__FLASH__ = "Strom-WP";
const char strom_h[]				__FLASH__ = "Strom-H";
const char strom_datum[]			__FLASH__ = "Strom-Datum";
const char wp_uhrzeit[]				__FLASH__ = "WP-Uhrzeit";
const char wp_betriebsart[]			__FLASH__ = "Betriebsart";
const char wp_einmal[]				__FLASH__ = "WW-Einmal";

const vito_param_t vito_params[] __FLASH__ = {
...
const char * const read_params[] __FLASH__ = {
...
{
	strcpy(param_name, vito_params[in].name);
#if _DEBUG_>1
	Serial.print((int)in); Serial.print(": Vito_GetParamName: "); Serial.println(param_name);
#endif
	return param_name;
}
/*****************************************************************************/
...
{
#if _DEBUG_>1
	Serial.print("Vito_LoadParam:"); Serial.println((int)in);
#endif
	memcpy(&vito_param, &vito_params[in], sizeof(vito_param_t));
#if _DEBUG_>1
	sprintf(spf, "vito_param: name: %s, addr:%4X, len: %u, divid: %u\n", vito_param.name, vito_param.addr, vito_param.len, vito_param.divid); Serial.print(spf);
#endif
}
/*****************************************************************************/
/*****************************************************************************/
...
{
#if _DEBUG_>1
	Serial.print("GetKeyIndex key: "); Serial.println(key);
#endif
//check if key exists
	for (byte i=0; i<sizeof(vito_params)/sizeof(vito_param_t); i++) {
		//if ( key==vito_params[i].name )
	#if _DEBUG_>1
		sprintf(spf, "vito_params[%i]: %s\n", (int)i, vito_params[i].name); Serial.print(spf);
	#endif
		if ( strcmp(key, vito_params[i].name)==0 )
			return i;
	}
#if _DEBUG_>1
	Serial.println("GetKeyIndex found nothing.");
#endif
	return -1;
}
/*****************************************************************************/
