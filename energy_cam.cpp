...
typedef struct {
	...
	unsigned char regs_high;
	unsigned char regs_low;
} modbus_head_t;

const modbus_head_t ec_frames[] __FLASH__ = {
// all integer values are byte-swapped because of storing in little-endianess
{MODBUS_EC_ADDRESS, EC_INPUT_REG_READ_MULTIPLE,  0x00,	0x1f, 0x00,	0x01},	// read status register
{MODBUS_EC_ADDRESS, EC_HOLDING_REG_WRITE_SINGLE, 0x00,	0x21, 0x00,	0x01},	// start OCR                  1 6 0 0x21 0 1 0x18 0
{MODBUS_EC_ADDRESS, EC_INPUT_REG_READ_MULTIPLE,  0x00,	0x21, 0x00,	0x01},	// read OCR status
{MODBUS_EC_ADDRESS, EC_INPUT_REG_READ_MULTIPLE,  0x00,	0x43, 0x00,	0x03},	// read OCR result, 2 words:  01 04 00 0x43 00 02 0x80 0x1F
};
...
#ifdef USE_RS485
 #if _DEBUG_>0
	Serial.println(("Connected to EnergyCam via RS485."));
 #endif
	RS485_ENABLE_TX;
	ec_state = EC_OK;
#else
		...
		while ( ec_client.available() )
		{
			...
			if ( mb_state==RECEIVING_DATA )	{
				...
				if ( buffer>=BUFFER_SIZE )
					...
					ProcessError(WRONG_DATA_LEN);	// wrong data length
			}
