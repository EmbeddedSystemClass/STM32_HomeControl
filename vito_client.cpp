...
#if _DEBUG_>1
	Serial.println("VitoClient_ResetPoll...");
#endif
...
#ifdef USE_RS485
  #if _DEBUG_>1
		Serial.println("writing 16 00 00...");
		DEBUG_PIN_LOW;
  #endif
...
byte VitoClient_SendGet(void)
{
	...
	while ( (retry--)>0 && ret==0 ) {
		...
		// wait for reply
		ret = VitoClient_GetReply(0);	//return VitoClient_GetReply(0);
	}
	return ret;	// retries exhausted, return error
}
/*******************************************************************************/
...
void VitoClient_ReadParameters(void)
{
	...
	int8_t index;
	while ( j<7 && (index = GetKeyIndex(read_params[j]))>=0 )	// number should be updated according to size of 'read_params[]'
	{
