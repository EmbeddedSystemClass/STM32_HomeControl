...
{
#if _DEBUG_>1
  Serial.print("SendNTPpacket - begin packet.");
  	showSocketStatus();
#endif
  ...
  Udp.endPacket();
#if _DEBUG_>1
  Serial.println(".SendNTPpacket - end.");
  showSocketStatus();
#endif
}
/****************************************************************************/
...
typedef union {
  ...
  IPAddress ntpServer(192, 168, 100, 1);  // Fritz.Box from intranet
#if _DEBUG_>1
	Serial.print(("Transmitting NTP Request, UDP.begin..."));
	showSocketStatus();
#endif
  Udp.begin(localPort);
//  delay(1000);
  for ( counter=0; counter<10; counter++)
  {
...
#ifdef _DEBUG_
    Serial.print(("transmitting NTP packet..."));
#endif
	...
	delay(1000);
    timeout = 0;
    ctime = millis();  // wait up to 1.5 seconds for the answer
    while ( (size=Udp.parsePacket())<NTP_PACKET_SIZE && timeout==0 ) {
      if ( (millis()-ctime)>1500 ) timeout = 1;
    }
...
#ifdef _DEBUG_
    Serial.print(("Received NTP time: "));
