...
#if _DEBUG_>0
		Serial.println(("DM parsed payload line: XML header removed."));
#endif
	} else {
...
#if _DEBUG_>0
	Serial.print(("Dreambox received: ")); Serial.println(s_buf);
#endif
}
/*****************************************************************************/
...
void Dreambox_Connect(void)
{
	...
	if ( reply<=0 ) {
		...
		Serial.print(("failed: "));
		if ( reply==-1 ) Serial.println(("timed out."));
		else if ( reply==-2 ) Serial.println(("invalid server."));
		else if ( reply==-3 ) Serial.println(("truncated."));
		else if ( reply==-4 ) Serial.println(("invalid response"));
...
{
	strcpy(s_buf, ("POST /web/timerlist HTTP/1.1\r\n"));
...
{
	strcpy(s_buf, ("POST /web/timercleanup?cleanup=true HTTP/1.1\r\n"));
	dm_client.println(s_buf);
/* reply:
<?xml version="1.0" encoding="UTF-8"?>
<e2simplexmlresult>
